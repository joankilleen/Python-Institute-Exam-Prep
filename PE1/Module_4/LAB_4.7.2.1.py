...
def enter_move(board):
    ...
    for tup in free_fields:        
        if move == board[tup[0]][tup[1]]:
            board[tup[0]][tup[1]] = MOVE_BY_USER
            return
...
def make_list_of_free_fields(board):    
    ...
    for x in range(len(board)):
        for y in range(len(board[0])):
            if board[x][y] != MOVE_BY_USER and board[x][y] != MOVE_BY_COMPUTER:
...
def draw_move(board):
    tup=()
    free_fields = make_list_of_free_fields(board)
    if BEST_POSITION in free_fields:
        tup=(1,1)
    else:
        tup = random.choice(free_fields)
    board[tup[0]][tup[1]] = MOVE_BY_COMPUTER
...
while free_fields != [] and victory==False:
    ...
    print(free_fields)
    enter_move(board)
    victory = victory_for(board, MOVE_BY_USER)
    if (victory):
    ...
    if free_fields != []:
        draw_move(board)
    victory = victory_for(board, MOVE_BY_COMPUTER)
    if (victory):
